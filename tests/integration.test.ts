...
// Runs against the real API only when WEBSHARE_API_KEY is set. An optional
// WEBSHARE_BASE_URL overrides the production base URL.
describe.skipIf(apiKey === undefined || apiKey === '')('integration (real API)', () => {
  test('profile get + proxies list', async () => {
    const client = new Webshare({ baseURL: process.env['WEBSHARE_BASE_URL'] });
