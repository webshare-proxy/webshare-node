...
describe('pagination', () => {
  test('exposes the raw envelope', async () => {
    ...
    server.respond(
      page([{ id: 1 }, { id: 2 }], { count: 5, next: `${server.url}/api/v2/apikey/?page=2`, previous: null }),
    );
  ...
  test('async iteration crosses page boundaries following next verbatim', async () => {
    const client = makeClient();
    server.respond(
      page([{ id: 1 }, { id: 2 }], { count: 5, next: `${server.url}/api/v2/apikey/?page=2&page_size=2` }),
      page([{ id: 3 }, { id: 4 }], { count: 5, next: `${server.url}/api/v2/apikey/?page=3&page_size=2` }),
      page([{ id: 5 }], { count: 5, next: null }),
    );
    const ids: number[] = [];
    for await (const key of await client.apiKeys.list({ page_size: 2 })) {
      ids.push(key.id);
    }
    expect(ids).toEqual([1, 2, 3, 4, 5]);
    expect(server.requests).toHaveLength(3);
    // The follow-up requests hit the exact next URLs, verbatim.
    expect(server.requests[1]?.url).toBe('/api/v2/apikey/?page=2&page_size=2');
    expect(server.requests[2]?.url).toBe('/api/v2/apikey/?page=3&page_size=2');
  ...
  test('nextPage() fetches the next page; throws when exhausted', async () => {
    const client = makeClient();
    server.respond(
      page([{ id: 1 }], { count: 2, next: `${server.url}/api/v2/apikey/?page=2` }),
